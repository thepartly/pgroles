...
- **Redacted JSON plans also redact comments and role configuration values.** `pgroles diff --format json` (single manifests and `pgroles.bundle_plan.v2` bundles) previously replaced only password values. It now also replaces role comments and `config` values with `"[REDACTED]"`, as `--format markdown` does; keys and shape are unchanged. `--format sql` still prints comments and configuration values and never prints password values. Read comment and configuration values from the manifest rather than from `diff` JSON. (#236)
- **`pgroles-core` has a default `passwords` feature.** It gates the `scram` module, `diff::resolve_passwords`, `diff::inject_password_changes`, and their SCRAM dependencies. Default builds are unchanged. Crates that set `default-features = false` must enable `passwords` to keep these APIs; parsing, planning, SQL rendering, visualization, authoring, and review artifacts remain available without it. (#236)
- **Browser authoring endpoints limit policy YAML to 1 MiB.** `authoring::validate_policy` and `authoring::compile_policy` return a `policy_too_large` diagnostic above `authoring::MAX_POLICY_YAML_BYTES`, and the explorer's `analyze` applies the same limit. `pgroles validate`, `diff`, and `apply`, bundles, the operator, and `authoring::prepare_policy` have no byte limit; manifest entry bounds still apply everywhere. Rust code that matches `ManifestError` exhaustively needs an arm for the new `YamlTooLarge` variant. `pgroles_cli::ValidatedManifest` is now an alias of `authoring::PreparedPolicy`, with the same fields. (#236, #241)
- **New `diff` review flags need this release.** `--review-out`, `--target-label`, `--policy-commit`, and `--executor-role` are usage errors in 0.12.0 and earlier, and usage errors exit with code 2 like drift. Pin the image or binary version in CI jobs that use them, and accept exit code 2 as drift only when the review file was written, as the updated [PR-comment recipe](https://thepartly.github.io/pgroles/docs/ci-cd/#diff-as-a-pr-comment) does. (#236, #241)
- **Recorded reviews use `pgroles.review-artifact.v2`.** Files exported by development builds of `main` as `pgroles.review-artifact.v1` are rejected by the explorer; export them again. (#241)
- **`pgroles-wasm` is new and unpublished** (`publish = false`). It is built by the documentation workflow and is not released to crates.io or as a package. (#236)
- **Explorer request additions are optional.** `analyze` requests accept `pg_major_version` (default `16`), `authority_graph_complete` (default `true`), and `executor.createrole` (default `unknown`), so requests written for #236 keep their meaning. `pg_major_version` accepts 12 through 20 and other values are rejected; the response echoes the effective `pg_major_version` and `authority_graph_complete`, and aggregated findings carry `change_indices`. Recorded reviews and the explorer both use these fields. (#241)

### Added

- **Browser plan explorer.** The documentation includes an explorer that compares desired YAML with a caller-sanitized snapshot and shows the ordered plan, phase-by-phase executor authority, findings, and the resulting role graph. `SET ROLE`, inherited privileges, and `ADMIN OPTION` are modelled separately; checks the snapshot cannot answer are reported as database preflight requirements. It runs `pgroles-core` as WebAssembly in the browser, never connects to PostgreSQL or applies SQL, rejects password sources, and labels its plan fingerprint as illustrative. Guides open five bundled Acme scenarios by ID. (#236)
- **Explorer analysis is faster and more precise.** Reachability is indexed by member and recomputed only after role creation or removal, membership changes, or `SUPERUSER`/`INHERIT` changes, so a 1000-role plan analyzes in milliseconds rather than tens of seconds. `database_preflight_required` is reported once per phase and change kind with `change_indices`, and `SetPassword` now counts. New findings: `superuser_required` (error) when a non-superuser executor grants or revokes membership in a `SUPERUSER` role, and `ownership_transfer_changes_grantor` (info) naming the old and new schema owner. A missing `ADMIN OPTION` path is an error when the authority graph is complete; before PostgreSQL 16, `executor.createrole: allowed` authorizes membership changes, and every membership proves `SET ROLE` because the per-membership `SET` option does not exist there. Dropped roles are no longer reported as access loss; an executor fact left `unknown` no longer overrides what the snapshot proves; `executor.superuser: true` wins over a snapshot entry without the flag; duplicate snapshot memberships, grants, and default privileges merge as inspection does. (#241)
- **Explorer controls and imports.** The explorer has a PostgreSQL major version select (15 to 18, default 16) and an executor `CREATEROLE` control, shows the version and authority-graph completeness the analyzer used, and accepts both from imported envelopes. Change labels name the affected grantee or member. The superuser control mirrors the analyzer: a snapshot `SUPERUSER` is fixed, and an explicit `executor.superuser: true` from an imported envelope is kept, marked **Overrides snapshot**, and can be cleared when the snapshot lists the role without the flag; typing a role the snapshot describes starts from that role's flag. Error alerts distinguish import failures, analyzer load failures, and analysis failures; a failed WebAssembly download can be retried in the same tab, and assets load with a content-hash build id so a cached glue file is never paired with a new binary. (#241)
- **Recorded-review import validation.** The explorer opens only `pgroles.review-artifact.v2` and rejects unknown fields anywhere in the file, keys named like credentials, an available SQL preview alongside omitted sensitive values, `complete` authority coverage with no recorded checks, comments on visual graph nodes, visual graph metadata, nodes, or edges whose kinds, flags, or counts do not have the schema's types, and malformed reachability deltas or finding indices. A documentation test derives its enum lists and key sets from the generated JSON Schema so importer and exporter cannot drift silently. (#241)
- **`pgroles diff --review-out`** writes a sanitized recorded review from the same inspection and planning run as the normal output, with any `--format`. The file records the ordered changes, bundle attribution, SQL preview, phase authority, findings, lifecycle-managed scope (`external: true` roles excluded), tool and PostgreSQL versions, capture time, policy content digest, and preflight evidence that names the targeted checks performed and the changes they did and did not cover. `--target-label` names the environment and rejects values containing `://`; `--policy-commit` records the source revision; `--executor-role` records an intended executor without verifying it. Password values, role configuration values, and comments are omitted with explicit omission records, and the whole SQL preview is omitted when any change carries them. The artifact never reads `password.from_env` variables. The explorer opens these files without loading WebAssembly or replanning. (#236, #241)
- **Review fingerprints link reports to files.** With `--review-out`, the Markdown report ends with the artifact's review fingerprint, and stderr prints the file path and the same fingerprint. If the file cannot be written, `diff` still prints the report, then exits with code 1, including with `--no-exit-code`. (#241)
- **Policy authoring API.** `pgroles_core::authoring::prepare_policy` is the parse, expansion, and normalization step shared by the CLI and explorer. `validate_policy` and `compile_policy` accept a versioned `PolicyRequest` (`pgroles.policy.v1`) and return structured diagnostics with stable codes, manifest field paths, and syntax byte ranges; compilation also returns the expanded policy and normalized desired graph. Neither resolves password sources. `diff::plan_changes` is the canonical planner for retirements, reconciliation mode, external roles, and preserved grants, shared by the CLI, operator, and explorer. (#236)
- **Review artifact contract.** `pgroles_core::review_artifact` types implement `Deserialize` and `JsonSchema`, and `parse_review_artifact` checks the size limit, schema version, change-index partition, reachability deltas, and fingerprint. The JSON Schema is generated to `docs/public/generated/review-artifact.schema.json` and served with the documentation. (#241)
- **Generated browser contracts.** Manifest metadata, the review artifact schema, and TypeScript authoring types are generated from Rust schemas by `scripts/generate-manifest-metadata.sh`. CI fails when they drift (`scripts/check-manifest-metadata.sh`) or when native and WebAssembly analysis disagree on shared fixtures (`scripts/check-wasm-parity.sh`, which honours `CARGO_TARGET_DIR`). (#236, #241)
- **`pgroles-wasm`**, a WebAssembly adapter that exposes `validate`, `compile`, and `analyze` to the documentation site. It builds `pgroles-core` without the `passwords` feature. (#236)
- **Row-level security lesson.** The Acme PostgreSQL course adds an advanced lesson between membership mechanics and the security review. Its labs use PostgreSQL to demonstrate default deny, tenant row visibility, rejected writes, owner and `FORCE` behaviour, bypass roles, and policy composition. Six cumulative chapter policies preserve earlier access. (#236)

### Changed

- **Recorded review phases store executor reachability as deltas.** Each phase lists only the roles whose `SET ROLE` or inherited-privilege status is new, changed, or removed since the previous phase (`executor_reachability_delta` and `executor_usage_delta`), instead of every role in every phase. A plan that creates 150 login roles with passwords previously produced a 4.5 MB file and failed against the 4 MiB import limit. (#241)
- **`diff` no longer computes password verifiers.** Every `diff` format redacts password values, so plans carry declared password intent. `sql`, `json`, and `summary` still fail when a declared `password.from_env` variable is unset; `markdown` does not read them. Output is unchanged. (#241)
- **Documentation navigation separates Docs, Learn PostgreSQL, and Explorer.** A canonical page registry drives sidebars, breadcrumbs, and reading sequences; product docs are grouped by task; the homepage routes to trying pgroles, adopting a database, and reviewing a plan. New planning and recorded-review guides replace sections of the CI/CD guide, whose anchors keep working. (#237)
...
- Optional policy-state monitoring examples include alert rules, Collector configuration, and operational queries. (#226)
...
- Policy-wide `role_pattern`, inherited by schema bindings with explicit per-schema overrides. CLI bundles also support `shared.role_pattern` and fragment overrides; candidates preserve the same naming content (#97).
...
- **The approval digest encoding is now `pgroles.io/approval-effect/v3`**, binding the target database's identity and default-privilege scopes into every approval. On the first reconcile after upgrading, every open plan is superseded and replaced by an equivalent plan under v3; recorded decisions do not carry over, so open plans need one fresh approval. Nothing executes in the meantime. Deliberately, a `pg_upgrade` (fresh `system_identifier`) or a blue-green cutover also invalidates any approval open across it — re-approve the fresh plan afterwards. (#180)
- **Policy content now has explicit size limits.** Identifiers are capped at 63 characters and 63 bytes (where PostgreSQL silently truncates), and every list and map has a bound — 1024 roles, 4096 grants, 2048 memberships, and so on (full table in the [manifest reference](https://thepartly.github.io/pgroles/docs/manifest-reference/)). A policy exceeding a limit is rejected on its next apply with a field-level error; each limit sits at least 20× above the largest policy known to run in production. (#182, #173)
- **Bundle plan JSON is now `pgroles.bundle_plan.v2`**, and `diff --format json` changes shape the same way: default-privilege changes carry a tagged `scope` (`{"type": "schema", "schema": "app"}` or `{"type": "global"}`) in place of the bare `schema` string. **Migration:** read `scope.schema` where you read `schema`, and handle `scope.type == "global"` entries having no schema at all. The `diff` JSON output remains unversioned — treat its shape as unstable and pin the pgroles version if you parse it.
- **`spec.mode: plan` is renamed to `spec.mode: observe`.** The old value keeps working with identical behaviour through a deprecation window (a `ModeValueDeprecated` condition, an operator-log warning, and the `pgroles.deprecated.mode_plan` metric); a future release removes it. Update manifests at your convenience.
- **`password_valid_until` must be a UTC whole-second timestamp** (`YYYY-MM-DDTHH:MM:SSZ`). An offset or fractional-second timestamp is now a field-level validation error — it never converged before (see Fixed below).
...
- **Revokes target each grant's recorded grantor, so delegated grants converge.** PostgreSQL's [`REVOKE`](https://www.postgresql.org/docs/current/sql-revoke.html) removes only grants attributed to the revoker (a superuser acts as the object owner, or the bootstrap superuser for role memberships), so a privilege granted onward by a delegate — or a membership edge granted by another admin — survived every plain revoke, superuser executors included, and re-planned forever. Inspection now records the grantor of every ACL entry and membership edge (multiple edges per role/member pair are aggregated for diffing and revoked individually), and rendered revokes act as that grantor: `SET ROLE grantor; REVOKE ...;` for object privileges (restoring the connection's configured execution role afterwards), `REVOKE ... GRANTED BY grantor` for memberships. Delegated grants to `PUBLIC` converge the same way. The plan preflight verifies per grantor that the executor can become it and reports what is missing (`UnsatisfiableRevoke`: `diff` warns, `apply` blocks), checking authority phase by phase against the plan's own membership changes so an executable plan is not falsely rejected. Object-ACL grantor targeting works on every supported version; membership targeting uses the per-edge grantor PostgreSQL 16 records, and older servers keep plain membership revokes. Grantor-less plan digests and JSON output are unchanged. See [limitations](https://thepartly.github.io/pgroles/docs/limitations/#grant-options-and-effective-access).
- **`PostgresPolicyCandidate`: propose, review, and promote policy content without touching the live policy.** A candidate points at an existing `PostgresPolicy` and carries only proposed content; execution settings come from the parent. Candidates are immutable once created — the version reviewed is exactly the version approved; revise by filing a successor with `replaces`. Each open candidate gets its own `PostgresPolicyPlan`, computed with the parent's credentials and locks against post-enforcement state, reviewed and decided like any other plan — and candidate planning never writes: no SQL in any state, no generated-password Secrets. Merging an approved candidate's content makes its reviewed plan the one that executes: the operator adopts the candidate's plan (never minting an approval of its own) and executes only if the recomputed effects still match the approved digest; anything else is reported on the candidate (`PromotedWithoutApproval`, `PromotionDigestMismatch`, `PromotionNotExecuted`) rather than ignored. See the [candidate docs](https://thepartly.github.io/pgroles/docs/operator-candidates/) for the `kubectl` and CI recipes. (#182, #173)
- **`pgroles candidate` covers the review side of the candidate workflow.** `create` files a candidate from an ordinary manifest, validated locally through the same path as `pgroles validate` and created with `generateName` so concurrent proposals never collide; `list`, `status`, and `diff` show every candidate's phase and digest, expand one down to its plan's decision and promotion state, and print the SQL approving it would run. Deciding a plan stays `kubectl`-shaped on purpose: it is a status write gated by admission, so `decidedBy` records an authenticated identity. (#189)
- **Approvals are bound to the database they were reviewed against.** Every plan records the server's physical identity (`pg_control_system().system_identifier`) and a logical fingerprint of the resolved host, port, and database; if either changes between approval and execution, the plan is superseded instead of executed. Set `spec.connection.requirePhysicalIdentity: true` to stop reconciliation entirely (`TargetIdentityBlocked`) when the identifier cannot be read. URL-mode connections bind the endpoint they resolve to, not only the Secret name — editing the URL inside a referenced Secret is no longer invisible to an open approval, while credential rotation still does not invalidate one. (#180, #185)
- **Manage memberships in PostgreSQL predefined roles and `external: true` roles.** Membership rules can grant roles such as `pg_monitor` directly, without adding them to `roles`. By default pgroles manages only the declared members; set `exclusive: true` to revoke other ordinary members as well. PostgreSQL's built-in `pg_*` hierarchy is always preserved. `generate` exports predefined-role memberships, `inspect` reports them, and the preflight checks the executor's `ADMIN OPTION` and the role's availability on the connected PostgreSQL version. (#200)
...
- **Owner-wide default privileges.** `default_privileges` entries accept `scope: {type: global}` beside the `schema:` shorthand, emitting `ALTER DEFAULT PRIVILEGES FOR ROLE ...` with no `IN SCHEMA` clause — the layer that applies to every schema an owner creates objects in. Inspection reads the global layer for exactly the declared `(owner, object type)` pairs, reporting the *effective* default, and excludes owner self-entries so authoritative mode never revokes an owner's own default. See [default privileges](https://thepartly.github.io/pgroles/docs/default-privileges/).
- **`ensure: absent` and a typed `PUBLIC` grantee.** Grant and default-privilege entries accept `ensure: absent`, which revokes a privilege where it is held, and `role: PUBLIC` addresses the pseudo-role — closing the gap where PostgreSQL's built-in `EXECUTE` for `PUBLIC` on every function could never be taken away declaratively. Inspection reports PUBLIC's *effective* privileges, synthesizing `acldefault(...)` where the ACL is still NULL. PUBLIC is reconciled only where a rule names it, in every mode: unmentioned PUBLIC privileges are never revoked, and deleting a `present` PUBLIC rule does not revoke it — switch the rule to `ensure: absent`. `additive` ignores absence assertions with a warning; profiles reject them by name. See [grants](https://thepartly.github.io/pgroles/docs/grants/).
...
- **Interactive PostgreSQL access guide.** The documentation now includes a browser-based course covering roles, ownership, grants, default privileges, drift, offboarding, and security review, with runnable exercises that need no local PostgreSQL. (#197)

### Changed

- **Plan retention is bounded per phase, so replan churn no longer evicts the record of what ran.** The bounds are `Applied` 25 (never fewer than 30 days' worth, hard ceiling 200, measured by `status.appliedAt`), `Failed` and `Rejected` 10 shared, `Superseded` 3; live phases are never evicted, and `pgroles.io/keep=true` still exempts a plan. Each bound is operator-level configuration (`PLAN_RETENTION_*` environment variables, replacing the unusable `max_plans` parameter); an invalid value refuses operator startup. The same bounds govern terminal-candidate pruning, and `keep` on a candidate or its plan exempts the pair. (#194)
- **Planning every open candidate costs one database inspection instead of one each.** The operator reads the database once over the union of the candidates' scopes and derives each candidate's own scoped inspection in memory — each candidate's wildcard expansion and diagnostics stay entirely its own, `spec.target` overrides still inspect for themselves, and a shared-read failure falls back per candidate so a proposal can never break enforcement. `pgroles.candidate.inspections` and `pgroles.candidate.planning.duration` report the cost. (#191)
- **Open candidates are bounded by a budget and a TTL.** At most 32 open candidates per policy are planned per pass (oldest first, so a CI loop cannot evict proposals under review); the rest report `CandidateBudgetExceeded` until older ones are decided or expire — nothing is deleted. An open candidate nobody decides within 14 days becomes `Superseded=True, reason=Expired`. `pgroles.io/keep=true` exempts a candidate from the TTL but not the budget. (#191)
- **Generated password Secrets are created when the approved plan executes, not when it is proposed**, so a rejected or never-approved plan no longer leaves a credential in the cluster. Deleting a generated Secret still rotates the password on the next apply — with a `GeneratedSecretMissing` warning Event first instead of rotating silently. (#181, #174)
- **`spec.schemas`, `spec.roles`, and `spec.retirements` are now map-lists** (keyed by name), so server-side apply merges entries instead of replacing whole lists, and duplicate keys are rejected at `kubectl apply` time. `memberships`, `grants`, and `default_privileges` stay plain arrays. (#126)

### Fixed

- **The operator no longer re-applies a non-convergent plan several times a second.** A plan whose SQL succeeds without changing inspected state re-applied about once a second under `approval: auto`, because each replan minted a fresh plan and every plan status write woke the controller. The reconciler now detects that identical effects were just applied and still diff, reports `Ready=False` with reason `NonConvergentPlan`, and retries at the policy interval.
- **`password_valid_until` now converges.** Offsets and fractional seconds could never compare equal to the inspected UTC whole-second rendering, and `VALID UNTIL 'infinity'`/`'-infinity'` inspected as an empty string — each re-planned an `ALTER ROLE` on every run. `infinity` now inspects as "no expiration", `-infinity` stays distinguishable so one apply repairs it, and validation accepts exactly the convergent form.
- **Never revoke an object's privileges from its owner.** pgroles previously interpreted PostgreSQL's implicit owner privileges as ordinary grants and could plan revocations against the owner after another ACL was added. Owner access is now protected across every managed object type, including wildcard rules, and generated manifests omit these implicit privileges. (#201)
- The Kyverno plan-decision policy no longer identifies the operator by a hardcoded ServiceAccount name — decision rules exempt callers holding the logical `manage` verb on the parent `PostgresPolicy`, checked with a `SubjectAccessReview`, so one policy covers any operator install and the standing bypass for anyone able to impersonate the hardcoded account is gone. Every newly terminal decision still records the authenticated identity; under `approval: auto`, `decidedBy` names the operator's ServiceAccount. (#187, #179)
- A superseded plan names why it was superseded (effects changed, effects vanished, replaced, or the target moved) instead of always claiming the database changed, and superseding no longer tries to rewrite the decision recorded on the plan — the reviewer's record stays exactly as they left it. (#184, #185)
- Plan lifecycle robustness: a crash between retiring a plan and creating its replacement can no longer leave a policy with nothing actionable; `status.current_plan_ref` is cleared before its plan is retired; a `mode: observe` policy retires its pending plan when drift disappears out of band; and a change set waiting out its failure-retry window reports `PlanFailedRetryBackoff` instead of "awaiting approval". (#185)
- The plan approval documentation describes the mechanism that exists — a status-subresource decision with `decidedBy` — with working `kubectl` commands; `kubectl get pgplan -o wide` shows the digest a decision actually binds; and the operator's RBAC covers the candidate `patch` and `delete` it performs. (#182, #184)
...
- **TLA+ model for wildcard-grant convergence** (`correctness/races/Convergence.tla`). Verifies eventually-permanent convergence under fairness and a finite number of external `DROP+CREATE`s, and produces a recurring-drift counterexample under v0.7.0 semantics. (#104)
