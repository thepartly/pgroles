...
- **Routine grant targets are resolved during inspection.** An `ensure: present` rule for a function or procedure that does not exist, a bare name that matches several overloads, or a named-argument signature spelled differently from the catalog now fails inspection of that policy, listing the input-type signatures to use; the operator reports `InvalidRoutineTarget`. Previously `diff` planned a `GRANT` that failed on apply. `pgroles generate` now writes input-type signatures such as `process(integer)`; policies generated by earlier versions keep working. (#239)
...
- **Routine grants no longer churn on equivalent signatures.** Function and procedure grants are keyed by input types across inspection, planning, export, and revoke preflight, so `process(integer)` and the catalog's `process(job_id integer)` no longer produce a grant followed by a revoke of the same routine. PostgreSQL type aliases such as `int2` resolve to one target, and opposite present and absent rules for that target fail before apply. (#239)
