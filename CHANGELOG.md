...
<!-- TODO(main): replace #TBD with this fix PR's number. -->
...
- **Browser authoring endpoints limit policy YAML to 1 MiB.** `authoring::validate_policy` and `authoring::compile_policy` return a `policy_too_large` diagnostic above `authoring::MAX_POLICY_YAML_BYTES`, and the explorer's `analyze` applies the same limit. `pgroles validate`, `diff`, and `apply`, bundles, the operator, and `authoring::prepare_policy` have no byte limit; manifest entry bounds still apply everywhere. Rust code that matches `ManifestError` exhaustively needs an arm for the new `YamlTooLarge` variant. `pgroles_cli::ValidatedManifest` is now an alias of `authoring::PreparedPolicy`, with the same fields. (#236, #TBD)
- **New `diff` review flags need this release.** `--review-out`, `--target-label`, `--policy-commit`, and `--executor-role` are usage errors in 0.12.0 and earlier, and usage errors exit with code 2 like drift. Pin the image or binary version in CI jobs that use them, and accept exit code 2 as drift only when the review file was written, as the updated [PR-comment recipe](https://thepartly.github.io/pgroles/docs/ci-cd/#diff-as-a-pr-comment) does. (#236, #TBD)
- **Recorded reviews use `pgroles.review-artifact.v2`.** Files exported by development builds of `main` as `pgroles.review-artifact.v1` are rejected by the explorer; export them again. (#TBD)
...
- **Explorer request additions are optional.** `analyze` requests accept `pg_major_version` (default `16`), `authority_graph_complete` (default `true`), and `executor.createrole` (default `unknown`), so requests written for #236 keep their meaning. `pg_major_version` accepts 12 through 20 and other values are rejected; the response echoes the effective `pg_major_version` and `authority_graph_complete`, and aggregated findings carry `change_indices`. Recorded reviews and the explorer both use these fields. (#TBD)
