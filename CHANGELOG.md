...
- **Explorer controls and imports.** The explorer has a PostgreSQL major version select (15 to 18, default 16) and an executor `CREATEROLE` control, shows the version and authority-graph completeness the analyzer used, and accepts both from imported envelopes. Change labels name the affected grantee or member. Error alerts distinguish import failures, analyzer load failures, and analysis failures; a failed WebAssembly download can be retried in the same tab, and assets load with a content-hash build id so a cached glue file is never paired with a new binary. (#241)
