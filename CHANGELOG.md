...
- **Routine grant targets are resolved during inspection.** An `ensure: present` rule for a function or procedure that does not exist, a bare name that matches several overloads, or a named-argument signature spelled differently from the catalog now fails inspection of that policy, listing the input-type signatures to use; the operator reports `InvalidRoutineTarget`. Previously `diff` planned a `GRANT` that failed on apply for a missing routine or an ambiguous name; a misspelled argument name applied, because PostgreSQL ignores argument names in `GRANT`, and then churned as described under Fixed. `pgroles generate` now writes input-type signatures such as `process(integer)`; policies generated by earlier versions keep working. (#239)
