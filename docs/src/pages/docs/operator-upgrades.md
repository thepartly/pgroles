...
- Function and procedure grants are matched by input types. A policy whose routine grant names a routine that does not exist, a bare name shared by several overloads, or an argument list spelled differently from the catalog now reports `InvalidRoutineTarget` instead of planning a grant that fails on apply. The condition message lists the signatures to use, such as `process(integer)`. Policies generated by earlier `pgroles generate` versions keep working.
