...
- CRDs and chart values are unchanged. Continue using the chart's version-matched CRDs.
- Function and procedure grants are matched by input types. A policy whose routine grant names a routine that does not exist, a bare name shared by several overloads, or a named-argument signature spelled differently from the catalog now reports `InvalidRoutineTarget`. In 0.12 the first two planned a grant that failed on apply; the third applied, then lost its grant on every second reconcile in authoritative and adopt modes. The condition message lists the signatures to use, such as `process(integer)`. Policies generated by earlier `pgroles generate` versions keep working.
