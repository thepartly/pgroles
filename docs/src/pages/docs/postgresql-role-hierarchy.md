...
The lab exercises a live PostgreSQL role graph. The explorer starts from a [snapshot you prepare and scrub](/docs/explorer-snapshots) and shows the ordered planned changes; use it to compare what the policy would change, then return to the lab to prove the resulting database behavior.
