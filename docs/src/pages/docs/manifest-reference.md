...
role_pattern: "{schema}-{profile}" # Default naming convention; schema bindings can override
...
Set top-level `role_pattern` to name all roles generated by schema/profile bindings.
An explicit `schemas[].role_pattern` overrides it, including an explicit
`"{schema}-{profile}"`. When neither is set, the built-in pattern is
`"{schema}-{profile}"`.
...
schemas:
  - name: inventory
    profiles: [viewer]                  # inventory_viewer
  - name: legacy
    profiles: [viewer]
    role_pattern: "legacy-{profile}"   # legacy-viewer
```

Every declared pattern must contain `{profile}` and may contain `{schema}`.
Patterns are limited to 128 characters; expanded role names must fit PostgreSQL's
63-byte identifier limit. Changing a pattern changes the desired role names;
review the generated plan before applying it.

For bundles, resolution is schema override → fragment `role_pattern` → bundle
`shared.role_pattern` → `"{schema}-{profile}"`. See [bundle composition](/docs/bundle-composition).
In Kubernetes, set the policy-wide value at `PostgresPolicy.spec.role_pattern`,
or `PostgresPolicyCandidate.spec.content.role_pattern` for candidate content.
...
| `role_pattern` | string | Inherited top-level pattern, then `"{schema}-{profile}"` | Override the naming pattern for this schema's profile-generated roles |
