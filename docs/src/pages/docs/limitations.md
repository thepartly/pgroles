...
Predefined capabilities require more than an ACL review: `pg_read_all_data` and `pg_write_all_data` provide broad data access without per-object grants and without bypassing RLS; monitoring roles supply observation capabilities. `pg_database_owner` instead has implicit membership for the current database owner and cannot be granted. Explicitly grantable predefined-role **memberships** are manageable: a membership stanza may name a `pg_*` role directly, declared members converge, and `exclusive: true` asserts the complete member list — see [memberships](/docs/memberships#predefined-and-external-granted-roles). What stays out of scope: the role objects themselves are never created, altered, or dropped, and undeclared members are never revoked without an explicit `exclusive` assertion, so provider-granted memberships survive adoption. `pgroles inspect` reports all `pg_*` memberships informationally, and `pgroles generate` exports them. [The security review](/docs/postgresql-security-review#review-predefined-roles-by-capability) compares the categories.
